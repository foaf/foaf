...
public class Rendezvous extends DNSConstants
{
	...
	try {
		...
		out.addAnswer(new DNSRecord.Service(info.name, TYPE_SRV, CLASS_IN, DNS_TTL, info.priority, info.weight, info.port, info.name), 0);
		out.addAnswer(new DNSRecord.Text(info.name, TYPE_TXT, CLASS_IN, DNS_TTL, info.text), 0);
		out.addAnswer(new DNSRecord.Address(info.name, TYPE_A, CLASS_IN, DNS_TTL, info.getIPAddress()), 0);
		send(out);
		i++;
		nextTime += 225;
	    }
	...
	for (Enumeration e = in.questions.elements() ; e.hasMoreElements() ;) {
		...
		if (info != null) {
			...
			out.addAnswer(in, new DNSRecord.Service(q.name, TYPE_SRV, CLASS_IN | CLASS_UNIQUE, DNS_TTL, info.priority, info.weight, info.port, info.name));
		    }
			...
			out.addAnswer(in, new DNSRecord.Address(q.name, TYPE_A, CLASS_IN | CLASS_UNIQUE, DNS_TTL, info.getIPAddress()));
		    }
		}
	    } 
	}
